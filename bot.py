...
BOT_TOKEN = "8283286774:AAEY6R72BHGHg-ef5CkSDF_wyWFtw-Tu_Nk"
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if audio.tags is None:
            audio.add_tags()
            audio.save()
            logger.info("Созданы новые ID3 теги для файла")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Обрабатываем MP3 файл - ОТКРЫВАЕМ ЗАНОВО для гарантии
        audio = MP3(file_path, ID3=ID3)

        # Убедимся, что теги существуют
        if audio.tags is None:
            audio.add_tags()
            logger.info("Созданы теги для добавления обложки")

        # Удаляем ВСЕ старые обложки
        apic_keys = [key for key in audio.tags.keys() if key.startswith('APIC')]
        for key in apic_keys:
            del audio.tags[key]
            logger.info(f"Удалена старая обложка: {key}")
        ...
        # ВАЖНО: Сохраняем с v2_version=3 для совместимости
        audio.save(v2_version=3)
        logger.info("Обложка добавлена и файл сохранен")

        # ПЕРЕЗАГРУЖАЕМ файл для проверки
